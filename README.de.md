...
Dieses README beschreibt die aktuelle v3-Pipeline. Die Laufzeitspezifikation der Skill liegt in [skills/startup-india-goat/SKILL.md](skills/startup-india-goat/SKILL.md) und ist maßgeblich für das aktuelle Verhalten von Befehlen und Setup.
...
Der Symlink hält die Installation beim Bearbeiten mit deinem Arbeitsverzeichnis synchron – erneutes Kopieren entfällt. Für `claude.ai` baust du die `.skill`-Datei aus dem Quellcode: `bash skills/startup-india-goat/scripts/build-skill.sh` erzeugt `dist/last30days.skill`.

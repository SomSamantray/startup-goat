...
| LinkedIn | authorized: `LINKEDIN_LI_AT` (plus optional `LINKEDIN_JSESSIONID` / `LINKEDIN_BCOOKIE`) for the cookie-session company-profile adapter, or `LINKEDIN_ACCESS_TOKEN` bearer token; posts/search: `SCRAPECREATORS_API_KEY` + `INCLUDE_SOURCES` contains `linkedin` | LinkedIn company profile + posts (cookie-session), or LinkedIn posts + articles via ScrapeCreators (articles rank as high signal on person topics) | cookie-session = free, in-memory only, user's own session; SC = 10K free calls; power-user opt-in, not offered during first-run onboarding |
...
The `startup-india-goat` fork defaults to public-only, entity-isolated research. Configure `STARTUP_GOAT_MEMORY_DIR` (default `~/Documents/StartupIndiaGOAT/`) for Markdown, HTML, versioned JSON, sanitized raw evidence, and manifest bundles. Its source doctor is available through `python3 skills/startup-india-goat/scripts/startup_goat.py --doctor`; unavailable, gated, quota, browser, and schema-drift states are reported as coverage limits rather than no-results claims. Private, gated, mixed, unknown, or secret-tainted artifacts are not eligible for publication.

Use explicit consent for LinkedIn tokens, Tracxn access, or allowlist-only browser captures. Never persist cookies, browser storage, authorization headers, raw HTML, or credentials.
